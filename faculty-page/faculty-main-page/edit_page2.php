...
$query = "SELECT * FROM faculty2db WHERE idnum = '$idnum'";
...
<body>
    ...
    <?php
        if(isset($_POST['edit_students'])){
            ...
            $query = "UPDATE faculty2db SET name = '$name', idnum = '$idnum', yr_level = '$yr_level', block = '$block', semester = '$semester', subject_code = '$subject_code', units = '$units', grade = '$grade' WHERE idnum = '$idnum_new'";
            $result = mysqli_query($con, $query);

            if(!$result) {
                die("Query Failed: " . mysqli_error($con));
            } else {
                header("location: faculty2_class.php?edit_msg=Data has been updated!");
    ...
    <div class="form">     
        <form action="edit_page2.php?idnum=<?php echo $idnum ?>" method="post">
